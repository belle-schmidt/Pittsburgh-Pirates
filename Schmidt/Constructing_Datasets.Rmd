...
```{r}
# calculates how many times each pitch type was thrown each year
PitchUse_Year <- Statcast_Combined_Pitches |> 
  group_by(pitch_name, year) |> 
  summarise(times_thrown_in_year = n()) 


# calculates how many many pitches total were thrown that year
TotalPitches_Year <- Statcast_Combined_Pitches |> 
  group_by(year) |> 
  summarise(pitches_thrown_in_year = n()) 

# calculates the proportion of times each pitch type was thrown in each year
...
# write.csv(PitchUse, "pitch_use.csv")
...
### Whiff Rate of Each Pitcher For Each Pitch Type in Each Year Between 2020-2024
...
Whiff_Rate <- left_join(Times_Swung, Times_Missed, by = c("pitcher" = "pitcher", "pitch_type" = "pitch_type", "year" = "year")) |> 
  mutate(num_missed = ifelse(is.na(num_missed), 0, num_missed),
         whiff_rate = num_missed / num_times_swung) |> 
  select(-num_missed, -num_times_swung)|> 
  ungroup() |> 
  pivot_wider(names_from = pitch_type, names_glue = "whiff_rate_{pitch_type}", values_from = whiff_rate)
...
Joined_FanGraphs_Statcast <- left_join(Joined_FanGraphs_Statcast, Whiff_Rate, by = c("xMLBAMID" = "pitcher", "Season" = "year"))
